...
cd $CUFFLINKS_DIR
#export LDFLAGS="-L$TACC_SAMTOOLS_LIB -lbam -Wl,-rpath,$TACC_BOOST_LIB -Wl,-rpath,$TACC_SAMTOOLS_LIB"
#export CXXFLAGS="-I$TACC_SAMTOOLS_INC -I$TACC_BOOST_INC"
./configure CC=gcc CXX=g++ --prefix=%{INSTALL_DIR} --with-boost=$TACC_BOOST_DIR --with-eigen=$CUFFLINKS_DIR/eigen --with-bam=$MY_SAMTOOLS_DIR --with-bam-libdir=$MY_SAMTOOLS_DIR LDFLAGS="-Wl,-rpath,$TACC_BOOST_LIB,-rpath,/opt/apps/gcc/4.4.5/lib64/"
# ./configure CC=gcc CXX=g++ --prefix=$CUFFLINKS_DIR/bin --with-boost=$TACC_BOOST_DIR --with-eigen=$CUFFLINKS_DIR/eigen --with-bam=$TACC_SAMTOOLS_DIR LDFLAGS="-Wl,-rpath,$TACC_BOOST_LIB"
make
mkdir -p $RPM_BUILD_ROOT%{INSTALL_DIR}
